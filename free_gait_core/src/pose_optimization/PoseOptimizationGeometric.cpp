...
bool PoseOptimizationGeometric::optimize(Pose& pose)
{
  ...
  if(hasNominalRotation_){
    // Rotation is known, calculate incremental translation

/*     Position ak(0.0,0.0,0.0);
    Position bk(0.0,0.0,0.0);

    // see (47)
    for (const auto& foot : stance_) {
      ak += foot.second;
      bk += nominalStanceInBaseFrame_.at(foot.first);
    }

    ak = ak / ((double) stance_.size());
    bk = bk / ((double) stance_.size());

    pose.getPosition() = ak - nominalRotation_.rotate(bk);
    pose.getPosition().z() = center.z(); */

    pose.getPosition() = center;

    pose.getRotation() = nominalRotation_;

/*     ROS_INFO_STREAM("Original position: " << center);
    ROS_INFO_STREAM("Optimised position: " << pose.getPosition()); */
