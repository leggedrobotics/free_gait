...
namespace free_gait_preview_rviz_plugin {

FreeGaitPreviewVisual::FreeGaitPreviewVisual(Ogre::SceneManager* sceneManager, Ogre::SceneNode* parentNode)
    : manualObject_(0),
      haveMap_(false)
{
...
void FreeGaitPreviewVisual::setMessage(const grid_map_msgs::GridMap::ConstPtr& msg)
{
...
void FreeGaitPreviewVisual::computeVisualization(float alpha, bool showGridLines, bool flatTerrain,
...
void FreeGaitPreviewVisual::setFramePosition(const Ogre::Vector3& position)
{
  frameNode_->setPosition(position);
}

void FreeGaitPreviewVisual::setFrameOrientation(const Ogre::Quaternion& orientation)
{
  frameNode_->setOrientation(orientation);
}

std::vector<std::string> FreeGaitPreviewVisual::getLayerNames()
{
  return map_.getLayers();
}

// Compute intensity value in the interval [0,1].
void FreeGaitPreviewVisual::normalizeIntensity(float& intensity, float min_intensity, float max_intensity)
{
...
Ogre::ColourValue FreeGaitPreviewVisual::getRainbowColor(float intensity)
{
...
Ogre::ColourValue FreeGaitPreviewVisual::getInterpolatedColor(float intensity, Ogre::ColourValue min_color,
